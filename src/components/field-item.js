...
const FieldItem = ({ name, active }) => {
  const [visible, setVisible] = useState(active);

  return (
    <View style={styles.container}>
      <TouchableOpacity onPress={() => console.log("Change order")}>
        <Ionicons name="menu" size={32} color={Colors.silverSand} />
      </TouchableOpacity>
      <View style={styles.wrapper}>
        <View style={styles.header}>
          <Image
            source={require("../assets/cancha-lorem.jpg")}
            style={styles.image}
          />
          <View style={styles.content}>
            ...
            <View style={styles.more}>
              <TouchableOpacity onPress={() => console.log("View more detail")}>
                <Fontisto
                  name="zoom-plus"
                  size={24}
